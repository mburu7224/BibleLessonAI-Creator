...
This is better than the original prototype, but it is not the final production architecture.

## Recommended Next Technical Upgrades

- Move server-side Firebase access to Firebase Admin SDK
- Replace single shared admin password with real user accounts
- Move Firebase config and collection naming to environment variables
- Add server-side request validation with a schema library
- Add automated tests for API routes and generation/export flows
